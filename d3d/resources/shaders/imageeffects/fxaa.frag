...
uniform sampler2D d3dTex;
uniform vec2 d3dRcpFrame;
uniform vec4 d3dRcpFrameOpt;

out vec4 d3dOutputColor;

void main() { 
	vec2 coord = gl_FragCoord.xy * d3dRcpFrame;

	vec2 pos = coord;
	vec4 posPos = vec4(pos.xy + d3dRcpFrame * 0.5, pos.xy - d3dRcpFrame * 0.5);

	d3dOutputColor = FxaaPixelShader(pos, posPos, d3dTex, d3dRcpFrame, d3dRcpFrameOpt);
}