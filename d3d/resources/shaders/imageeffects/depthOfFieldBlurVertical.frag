...
void main() {
	...
	sum += texture(d3dScene, vec2(coord.x, coord.y - 4.0 * blurSize)).rgb * 0.05;
	sum += texture(d3dScene, vec2(coord.x, coord.y - 3.0 * blurSize)).rgb * 0.09;
	sum += texture(d3dScene, vec2(coord.x, coord.y - 2.0 * blurSize)).rgb * 0.12;
	sum += texture(d3dScene, vec2(coord.x, coord.y - blurSize)).rgb * 0.15;
