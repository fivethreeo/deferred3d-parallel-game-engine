	...
	ComputeSystem::DeviceType computeDeviceType)
{
	...
	_computeSystem.create(computeDeviceType, true);
...
void RenderScene::useShader(Shader* pShader) {
	if (_shaderSwitchesEnabled) {
		...
		Shader::getCurrentShader()->setUniformmat4("d3dViewModel", _viewModelMatrix);
		Shader::getCurrentShader()->setUniformmat4("d3dProjectionViewModel", _projectionViewModelMatrix);
		Shader::getCurrentShader()->setUniformmat3("d3dNormal", _normalMatrix);
	}
...
void RenderScene::useShader(Material &material) {
	if (_shaderSwitchesEnabled) {
		...
		if (material._pDiffuseMap != nullptr)
			Shader::getCurrentShader()->setShaderTexture("d3dDiffuseMap", material._pDiffuseMap->getTextureID(), GL_TEXTURE_2D);
		else
			Shader::getCurrentShader()->setShaderTexture("d3dDiffuseMap", getWhiteTexture()->getTextureID(), GL_TEXTURE_2D);
...
void RenderScene::updateShaderUniforms() {
	...
	_gBufferRenderShaders[_standard]->setUniformmat4("d3dViewModel", _renderCamera._projectionMatrix);

	_gBufferRenderShaders[_normal]->bind();
	_gBufferRenderShaders[_normal]->setUniformmat4("d3dViewModel", _renderCamera._projectionMatrix);

	_gBufferRenderShaders[_heightNormal]->bind();
	_gBufferRenderShaders[_heightNormal]->setUniformmat4("d3dViewModel", _renderCamera._projectionMatrix);
...
void RenderScene::createMaterialInterface(UBOShaderInterface &materialInterface, Shader* pShader) {
	...
	materialInterface.create("d3dMaterial", pShader, materialNames, 5);
}