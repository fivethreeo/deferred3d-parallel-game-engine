#include <d3d/sceneobjects/map/Map3DWS.h>
...
bool Map3DWS::createAsset(const std::string &name) {
	...
	if (_settings._useGraphics) {
		...
		if (_settings._giRef != nullptr) {
			SceneObjectGI* pGI = static_cast<SceneObjectGI*>(_settings._giRef.get());

			pGI->addModel(*chunk->_model, Matrix4x4f::identityMatrix());
		}
	}
...
void d3d::addMapLights(const Map3DWS &map, Scene* pScene, const SceneObjectRef &gi) {
	...
	for (size_t i = 0; i < map._entities.size(); i++) {
		...
		if (entity._properties.find("classname")->second == "light") {
			...
			if (gi != nullptr) {
				SceneObjectGI* pGI = static_cast<SceneObjectGI*>(gi.get());

				pGI->addLight(pointLightShadowed);
			}
		}
	}
}