...
void Material::setUniformsBuffer(UBOShaderInterface &uboShaderInterface) {
	...
	uboShaderInterface.setUniformv3f("d3dDiffuseColor", _diffuseColor);
	uboShaderInterface.setUniformf("d3dSpecularColor", _specularColor);
	uboShaderInterface.setUniformf("d3dShininess", _shininess);
	uboShaderInterface.setUniformf("d3dEmissiveColor", _emissiveColor);
	uboShaderInterface.setUniformf("d3dHeightMapScale", _heightMapScale);

	VBO::unbind(GL_UNIFORM_BUFFER);
}

void Material::setUniforms(Shader* pShader, Texture2D* pWhiteTexture) {
	pShader->setUniformv3f("d3dDiffuseColor", _diffuseColor);
	pShader->setUniformf("d3dSpecularColor", _specularColor);
	pShader->setUniformf("d3dShininess", _shininess);
	pShader->setUniformf("d3dEmissiveColor", _emissiveColor);
	pShader->setUniformf("d3dHeightMapScale", _heightMapScale);
}

void Material::setUniformsTextures(Shader* pShader, Texture2D* pWhiteTexture) {
	switch (_type) {
	case RenderScene::_standard:
		pShader->setShaderTexture("d3dDiffuseMap", _pDiffuseMap == nullptr ? pWhiteTexture->getTextureID() : _pDiffuseMap->getTextureID(), GL_TEXTURE_2D);
		pShader->setShaderTexture("d3dSpecularMap", _pSpecularMap == nullptr ? pWhiteTexture->getTextureID() : _pSpecularMap->getTextureID(), GL_TEXTURE_2D);
		pShader->setShaderTexture("d3dShininessMap", _pShininessMap == nullptr ? pWhiteTexture->getTextureID() : _pShininessMap->getTextureID(), GL_TEXTURE_2D);
		pShader->setShaderTexture("d3dEmissiveMap", _pEmissiveMap == nullptr ? pWhiteTexture->getTextureID() : _pEmissiveMap->getTextureID(), GL_TEXTURE_2D);
	...
	case RenderScene::_normal:
		...
		pShader->setShaderTexture("d3dShininessMap", _pShininessMap == nullptr ? pWhiteTexture->getTextureID() : _pShininessMap->getTextureID(), GL_TEXTURE_2D);
		pShader->setShaderTexture("d3dEmissiveMap", _pEmissiveMap == nullptr ? pWhiteTexture->getTextureID() : _pEmissiveMap->getTextureID(), GL_TEXTURE_2D);
		pShader->setShaderTexture("d3dNormalMap", _pNormalMap == nullptr ? pWhiteTexture->getTextureID() : _pNormalMap->getTextureID(), GL_TEXTURE_2D);
	...
	default:
		...
		pShader->setShaderTexture("d3dEmissiveMap", _pEmissiveMap == nullptr ? pWhiteTexture->getTextureID() : _pEmissiveMap->getTextureID(), GL_TEXTURE_2D);
		pShader->setShaderTexture("d3dNormalMap", _pNormalMap == nullptr ? pWhiteTexture->getTextureID() : _pNormalMap->getTextureID(), GL_TEXTURE_2D);
		pShader->setShaderTexture("d3dHeightMap", _pHeightMap == nullptr ? pWhiteTexture->getTextureID() : _pHeightMap->getTextureID(), GL_TEXTURE_2D);
