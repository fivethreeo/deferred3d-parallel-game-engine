	...
	const std::shared_ptr<Texture2DArray> &normal)
{
	...
	_gBufferRenderShader->setShaderTexture("d3dDiffuseMapArray", _diffuse->getTextureID(), GL_TEXTURE_2D_ARRAY);
	_gBufferRenderShader->setShaderTexture("d3dNormalMapArray", _normal->getTextureID(), GL_TEXTURE_2D_ARRAY);
}

void VoxelTerrain::generate(void(*generator)(d3d::VoxelChunk &chunk, int seed), int seed, SceneObjectPhysicsWorld* pPhysicsWorld, float restitution, float friction) {
