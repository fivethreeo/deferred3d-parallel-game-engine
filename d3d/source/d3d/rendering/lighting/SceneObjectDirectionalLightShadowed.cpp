...
void SceneObjectDirectionalLightShadowed::updateUniformBuffer() {
	...
	pLighting->_directionalLightShadowedLightUBOShaderInterface->setUniformv3f("d3dDirectionalLightDirection", getRenderScene()->_renderCamera.getNormalMatrix() * _direction);
	pLighting->_directionalLightShadowedLightUBOShaderInterface->setUniform("d3dLightBiasViewProjections", sizeof(float) * 16 * _lightBiasViewProjections.size(), &_lightBiasViewProjections[0]);

	if (_needsUniformBufferUpdate) {
		pLighting->_directionalLightShadowedLightUBOShaderInterface->setUniformv3f("d3dDirectionalLightColor", _color);
		pLighting->_directionalLightShadowedLightUBOShaderInterface->setUniformi("d3dNumCascades", static_cast<GLshort>(_cascades.size()));
		...
		pLighting->_directionalLightShadowedLightUBOShaderInterface->setUniform("d3dSplitDistances", sizeof(Vec4f) * splitDistancesVec4fs.size(), &splitDistancesVec4fs[0]);
...
void SceneObjectDirectionalLightShadowed::setCascadeShadowMaps(Shader* pShader) {
	for (int i = 0; i < _cascades.size(); i++)
		pShader->setShaderTexture("d3dCascadeShadowMaps[" + std::to_string(i) + "]", _cascades[i]->getDepthTextureID(), GL_TEXTURE_2D);

	for (int i = _cascades.size(); i < 3; i++)
		pShader->setShaderTexture("d3dCascadeShadowMaps[" + std::to_string(i) + "]", 0, GL_TEXTURE_2D);
}