...
void SceneObjectDirectionalLight::updateUniformBuffer() {
	...
	pLighting->_directionalLightLightUBOShaderInterface->setUniformv3f("d3dDirectionalLightDirection", getRenderScene()->_renderCamera.getNormalMatrix() * _direction);

	if (_needsUniformBufferUpdate) {
		pLighting->_directionalLightLightUBOShaderInterface->setUniformv3f("d3dDirectionalLightColor", _color);
