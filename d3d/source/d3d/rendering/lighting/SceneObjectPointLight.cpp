...
void SceneObjectPointLight::updateUniformBuffer() {
	...
	pLighting->_pointLightLightUBOShaderInterface->setUniformv3f("d3dPointLightPosition", getRenderScene()->_renderCamera.getViewMatrix() * _position);

	if (_needsUniformBufferUpdate) {
		pLighting->_pointLightLightUBOShaderInterface->setUniformv3f("d3dPointLightColor", _color);
		pLighting->_pointLightLightUBOShaderInterface->setUniformf("d3dPointLightRange", _range);
		pLighting->_pointLightLightUBOShaderInterface->setUniformf("d3dPointLightRangeInv", 1.0f / _range);
