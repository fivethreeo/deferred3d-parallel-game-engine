...
void SceneObjectSpotLight::updateUniformBuffer() {
	...
	pLighting->_spotLightLightUBOShaderInterface->setUniformv3f("d3dSpotLightColor", _color);
	pLighting->_spotLightLightUBOShaderInterface->setUniformf("d3dSpotLightRange", _range);
	pLighting->_spotLightLightUBOShaderInterface->setUniformf("d3dSpotLightRangeInv", 1.0f / _range);
	pLighting->_spotLightLightUBOShaderInterface->setUniformf("d3dSpotLightSpreadAngleCos", _spotAngleCos);
	pLighting->_spotLightLightUBOShaderInterface->setUniformf("d3dSpotLightSpreadAngleCosFlipInv", 1.0f / (1.0f - _spotAngleCos));
	pLighting->_spotLightLightUBOShaderInterface->setUniformf("d3dSpotLightExponent", _exponent);

	if (_needsUniformBufferUpdate) {
		pLighting->_spotLightLightUBOShaderInterface->setUniformv3f("d3dSpotLightPosition", getRenderScene()->_renderCamera.getViewMatrix() * _position);
		pLighting->_spotLightLightUBOShaderInterface->setUniformv3f("d3dSpotLightDirection", getRenderScene()->_renderCamera.getNormalMatrix() * _direction);
