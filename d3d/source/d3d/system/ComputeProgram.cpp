#include <d3d/system/ComputeProgram.h>
...
bool ComputeProgram::createAsset(const std::string &name, void* pData) {
	...
	if (_program.build(std::vector<cl::Device>(1, pComputeSystem->getDevice()), "-cl-opt-disable -cl-mad-enable") != CL_SUCCESS) { // -cl-fast-relaxed-math
